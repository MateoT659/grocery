...
// PROPS: READ HERE TO LEARN HOW THIS WORKS

export type SettingsTabProps = {
    viewProps?: any;            //style props for themedview
    textProps?: any;            //style props for themedtext
    showIcon?: boolean;         //show icon or not
    iconProps?: any;            //style props for icon
    iconColor?: string | null;  //color of icon
    iconSize?: number;          //size of icon
    icon?: IoniconName;         //name of icon
    children?: React.ReactNode; //text to be shown or child components
    title?: string | null;      //title of tab
    subtext?: string | null;    //subtext to be shown

    //NOTE: only use one tab type at a time! 

    //Navigation tab    
    route?: string | null;       //route to navigate to
    
    //Link to website
    link?: string | null;         //link to website

    //expand to new div
    expand?: boolean;             //expand or not. uses children as content
    expandedByDefault?: boolean;  //expanded by default

    //Switch boolean tab
    switchValue?: boolean | null;             //value of radio button
    onSwitchChange?: (value: boolean) => void; //function to change radio button value

    //Dropdown tab
    dropdownValue?: string | undefined;         //initial value of dropdown
    dropdownLabels?: string[] | null;           //labels of dropdown
    dropdownValues?: string[] | null;            //values of dropdown
    onDropdownChange?: (value: string) => void; //function to change dropdown value

    //types to add:

    //possibly as own types? would require groups of tabs
    //radio buttons (choose one of many options) (Dark mode, Light mode, etc, different rows?)
    //checkbox (choose multiple options) (Show notifications, etc)

}


export default function SettingsTab(
    { showIcon = true, icon = 'help-circle', iconProps, iconColor, iconSize=24,
      viewProps, textProps, 
      title = null,
      children, subtext = null,
      route = null, 
      link = null,
      expand = false, expandedByDefault=false,
      switchValue = null, onSwitchChange = () => {},
      dropdownValue = undefined, dropdownLabels=null, dropdownValues = null, onDropdownChange = () => {}
      }: SettingsTabProps
) { 
    // dropdown animation

    let dropdownVals:any = [];
    if(dropdownLabels !== null && dropdownValues !== null){
      dropdownVals = dropdownValues.map((value, index) => {
        return {label: dropdownLabels[index], value: value}
      });
    }
    ...
    const themeActiveColor = useThemeColor({dark: '#333333', light:'#EEEEEE'}, 'background')
    const themeBackgroundColor = useThemeColor({dark:'#222222', light:'#EEEEEE'}, 'background')

    const Tab = ({children}: {children: React.ReactNode}) => (
      <ThemedView>
        <ThemedView style={[styles.settingsTab, viewProps]}>
            {showIcon && <Ionicons name={icon} style={[styles.tabIcon, iconProps]} size={iconSize} color={iconColor ?? themeTextColor}/>}
            
            {switchValue !== null && <Switch value={switchValue} onValueChange={onSwitchChange} style={styles.switch} trackColor={{ false: 'gray', true: '#4B9CD3'}} />}
            

            <ThemedText style = {[textProps, {minHeight:24, margin:!showIcon?8:0}, link !== null && {color: '#0090FF' }, subtext!==null && {marginTop: -15}]}
             type="default">{ title ?? (!expand && children)}</ThemedText>
            
            {subtext !== null && <ThemedText style = {[styles.subtext]} type="default">{subtext}</ThemedText>}
            ...
            {dropdownValues!==null && 
              ...
              placeholder = {dropdownVals.find((item: { value: string | undefined } )=>item.value === dropdownValue)?.label}
              placeholderStyle={[{color: themeTextColor}, styles.dropdownText]}
              selectedTextStyle={[{color: themeTextColor}, styles.dropdownText]}
              activeColor = {themeActiveColor}
              itemTextStyle={{color: themeTextColor}}
              containerStyle={[{backgroundColor: themeBackgroundColor, borderWidth:0, marginTop:-50, borderRadius:10}]}
              style={[{borderColor: themeTextColor}, styles.dropdown, {backgroundColor: themeBackgroundColor, borderRadius:10}]}
              />
              }
            {expand && <Ionicons name={expanded === true ? 'chevron-down' : 'chevron-back'} style={styles.rightArrowIcon} size={30} color={'white'} />}
            {route !== null && <EvilIcons name="chevron-right" style={styles.rightArrowIcon} size={30} color={'white'}/>}
            
        </ThemedView>
          {expand && expanded && <ThemedView style={styles.expandedTab}>{children}</ThemedView>}
      </ThemedView>
    )
   ...
   return route === null && link === null ? (
    ...
    <Link href = {{pathname: (link !== null ? link : route !== null ? route: '/') as any}} asChild>
        <Pressable>
            <Tab>{children}</Tab>
        </Pressable>
    </Link>
